const { check, runTest, runTest } = require("./test-api/index.js");
...
function addIsCool(devs) {
  return devs.map((dev) => ({
    ...dev,
    isCool: dev.name.toLowerCase().includes("ch")
  }));
}
runTest(
...
function reduceNumbersAndIndex(nums) {
  return nums.reduce((accumulator, currentValue, currentIndex) => accumulator + currentValue + currentIndex, 0);
}
runTest(
